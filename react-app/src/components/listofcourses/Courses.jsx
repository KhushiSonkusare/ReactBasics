...
const Course = props =>{
    const { id, imageUrl, title, rating, price } = props.coursedetails;
        let stars = [];
        for (let i = 0; i < rating; i++) {
        stars.push(
            <i key={i} className="fa-solid fa-star" style={{ color: "gold" }}></i>
        ...
        const [currentLikes, setCurrentLikes] = useState(props.coursedetails.likes);

    return (
        <div className="col-md-3 my-1">
        <div className="card" style={{ width: "18rem" }}>
            <Link to={`/coursedetails/${id}`} className="text-decoration-none text-dark">
            <img
            src={imageUrl}
            alt={title}
            width="200px"
            ...
            <button className="btn btn-primary" onClick = {()=>setCurrentLikes(currentLikes+1)} >
                {currentLikes}<i className="fa-solid fa-thumbs-up" style={{marginLeft:"10px"}}></i>
            </button>
            </div>
        </div>
        </div>
